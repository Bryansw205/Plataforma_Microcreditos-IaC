  ...
  dashboard_body = jsonencode({
    widgets = [
      ...
      },
      {
        ...
        height = 6
        properties = {
          query  = "SOURCE '${aws_cloudwatch_log_group.ecs_api.name}'\nfields @timestamp, @message, @logStream | filter @message =~ /(?i)(error|fail|warn|exception)/ | sort @timestamp desc | limit 50"
          region = var.aws_region
      ...
      },
      {
        ...
        height = 6
        properties = {
          query  = "SOURCE '${aws_cloudwatch_log_group.ecs_worker.name}'\nfields @timestamp, @message, @logStream | filter @message =~ /(?i)(error|fail|warn|process|start|complete)/ | sort @timestamp desc | limit 50"
          region = var.aws_region
      ...
      },
      {
        ...
        height = 6
        properties = {
          query  = "SOURCE '/aws/rds/cluster/${aws_rds_cluster.aurora.cluster_identifier}/postgresql'\nfields @timestamp, @message | filter @message =~ /(?i)(alter|drop|create|duration|statement|error|fatal)/ | sort @timestamp desc | limit 50"
          region = var.aws_region
      ...
      },
      {
        ...
        height = 6
        properties = {
          query  = "SOURCE '${aws_cloudwatch_log_group.cloudtrail.name}'\nfields @timestamp, eventName, eventSource, userIdentity.arn, errorCode, errorMessage | sort @timestamp desc | limit 50"
          region = var.aws_region
